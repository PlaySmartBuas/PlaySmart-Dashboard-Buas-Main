 ...
 * ValorantDashboard
 * -----------------
 * Shows a player's Valorant account summary and paginated match history.
 * Responsibilities:
 * - Resolve the Riot ID from URL or sessionStorage
 * - Load account info and store PUUID locally
 * - Fetch an initial batch of matches and support incremental display
 *
 * Notes:
 * - Uses `valorantAPI` wrapper for backend calls and `valorantStorage` for
 *   lightweight local persistence of the PUUID/account data.
 */
const ValorantDashboard: React.FC = () => {
  ...
  // Load the initial page of matches from the Valorant API wrapper.
  // We fetch MATCHES_TO_FETCH and display only MATCHES_PER_DISPLAY initially
  // to allow smooth incremental loading on the page.
  ...
  // Initialization effect: resolve Riot ID (URL or session), load account
  // summary and initial matches. This runs once per mount unless the
  // `isInitialized` flag is reset.
  useEffect(() => {
    ...
    const initializeAccount = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          // Fetch the player's recent match history after account load.
  ...
  // Expand the number of displayed matches. This uses the pre-fetched
  // `allFetchedMatches` array to avoid additional network requests while
  // the user pages through results.
